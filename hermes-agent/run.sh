...
# AUTO-MOUNT SHARE: Se esiste la map op de share, bind deze naar /data
if [ -d "/share/hermes_windows/data" ]; then
    echo "[Info] Windows Share gedetecteerd. Koppelen aan /data..."
    mount --rbind /share/hermes_windows/data /data
fi

# ROOT PERMISSIONS & GATEWAY FIX
...
# Zorg dat de data map van de juiste rechten is voorzien voor root
